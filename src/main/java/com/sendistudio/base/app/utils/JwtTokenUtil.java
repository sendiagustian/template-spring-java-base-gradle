...
public class JwtTokenUtil {
    ...
    public static class AccessToken {
        ...
        public SecretKey getSigningKey() {
            String SECRET_KEY = appProperties.getAppVars().getSecretToken();
        ...
        // Generate JWT token
        public String generateToken(String username, String userId, String role, Long expirationMillis) {
    ...
    public static class RefreshToken {
        ...
        public SecretKey getSigningRefreshKey() {
            String SECRET_KEY = appProperties.getAppVars().getSecretRefresh();
            return Keys.hmacShaKeyFor(SECRET_KEY.getBytes());
        }
        ...
        public Optional<String> extractUsernameRefresh(String token) {
            try {
                return extractClaimRefresh(token, claims -> claims.getSubject());
            } catch (ExpiredJwtException e) {
                // Jika token sudah expired, ambil klaim dari token yang sudah expired
                return Optional.ofNullable(e.getClaims().getSubject());
            } catch (Exception e) {
    ...
    public static class ForgotPssword {
        ...
        public SecretKey getSigningForgotPasswordKey() {
            String SECRET_KEY = appProperties.getAppVars().getSecretForgotPassword();
            return Keys.hmacShaKeyFor(SECRET_KEY.getBytes());
        }
        ...
        public Optional<String> extractUsernameForgotPassword(String token) {
            try {
                return extractClaimForgotPassword(token, claims -> claims.getSubject());
            } catch (ExpiredJwtException e) {
                // Jika token sudah expired, ambil klaim dari token yang sudah expired
                return Optional.ofNullable(e.getClaims().getSubject());
            } catch (Exception e) {
